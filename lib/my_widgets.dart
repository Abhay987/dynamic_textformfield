...
class _MyWidgetsState extends State<MyWidgets> {
  ...
  TextFormField _generateTextFieldNew(TextEditingController controller, String hint) {
  ...
  TextFormField _generateTextField(TextEditingController controller, String hint) {
    return TextFormField(
  ...
  List<Widget> _getFriends(){
    ...
    for(int i=0; i<invoiceList.length; i++){
      num? qty = num.tryParse(qtyControllers[i].text) == null ? 0 : num.tryParse(qtyControllers[i].text);
      num? price = num.tryParse(priceControllers[i].text) == null ? 0 : num.tryParse(priceControllers[i].text);
      num? disc = num.tryParse(discControllers[i].text) == null ? 0 : num.tryParse(discControllers[i].text);
      num? total = qty! * price!;
